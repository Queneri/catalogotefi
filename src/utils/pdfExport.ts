...
export const exportToPDF = async (products: Product[]) => {
  ...
  products.forEach((product, index) => {
    ...
    pdf.text(product.category.toUpperCase(), margin, yPosition);
    yPosition += lineHeight;

    // Sizes
    pdf.setTextColor(0, 0, 0);
    pdf.text(`Talles: ${product.sizes.join(", ")}`, margin, yPosition);
    yPosition += lineHeight;

    // Price
    pdf.setFontSize(11);
    pdf.setFont("helvetica", "bold");
    pdf.text(`$${product.price.toFixed(2)}`, margin, yPosition);
    yPosition += lineHeight;
